...
In modern browsers, you can specify `type='flex'`, and then you can use `align` and `justify` to achieve horizontal and vertical alignment in different ways, and you can also use `order` to change the order of content blocks (` order` also supports responsive).
