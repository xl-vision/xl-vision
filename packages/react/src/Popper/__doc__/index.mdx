...
> 这个组件很强大，使用这个组件包裹的参考组件不会在 dom 上产生任何包裹的元素，因此不会对参考组件产生任何影响。
> 同时，这个组件也只嵌套使用，子 popper 在逻辑上就像是属于父 popper 的弹出框内容的一部分，然而实际上子 popper 在 dom 结构上可以在任何位置。
> 这个组件的弹出框内容只在真正触发时才会挂载，因此性能有一定的保证，我们甚至可以使用这个组件实现无限层级的子父菜单项。
