...
> Dialog.useDialog has almost the same parameters and methods as method, but does not need to pass in themeContext and localeContext.

> We recommend using useDialog. This method can ensure that the theme and internationalization are consistent with the context, and will automatically destroy the dialog box when the parent component is destroyed, reducing the mental burden when using it.
