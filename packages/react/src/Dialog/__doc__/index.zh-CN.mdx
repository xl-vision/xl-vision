...
> 我们推荐使用`Dialog.useDialog`，这种方式可以保证主题和国际化与上下文一致，也会在父组件销毁的时候自动销毁对话框，降低了使用时的心智负担。

> `Dialog.useDialog`创建的对话框不会在关闭时自动销毁，而是在容器销毁时自动销毁，所以依然可以使用`update`方法显示出来。当然你也可以调用`destroy`来进行销毁。
